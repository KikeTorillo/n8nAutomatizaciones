...
import { FormDrawer, MultiSelect, Select } from '@/components/ui';
import {
  useCrearCita,
  useActualizarCita,
...
function CitaFormDrawer({
  ...
  const roundRobinHabilitado =
    configAgendamiento?.round_robin_habilitado || false;
  ...
  useEffect(() => {
    if (isEditMode && cita && isOpen) {
      ...
      reset({
        ...
        duracion_minutos:
          cita.duracion_total_minutos || cita.duracion_minutos || 30,
  ...
  const handlePreviewRecurrencia = async () => {
    ...
    if (
      ...
      toast.error(
        'Completa profesional, fecha, hora y duración para ver el preview'
      );
      return;
    }
  ...
  const onSubmit = async (data) => {
    try {
      ...
      const sanitized = {
        ...
        ...(data.profesional_id && data.profesional_id !== 'auto'
      ...
      if (isEditMode) {
        ...
        if (result.citas_omitidas?.length > 0) {
          toast.warning(
            `Serie creada: ${result.citas_creadas?.length} citas. ${result.citas_omitidas?.length} fechas omitidas por conflictos.`
          );
        } else {
      ...
      const mensajeError =
        ...
        `Error al ${accion}`;
      const mensajeFinal =
        mensajeError.startsWith('No se puede') ||
  ...
  const profesionalesOpciones = [
    ...(roundRobinHabilitado
      ? [{ value: 'auto', label: '🔄 Auto-asignar (Round-Robin)' }]
      : []),
    ...(profesionales || []).map((p) => ({
      value: p.id.toString(),
    ...
    >
      <div className="space-y-4">
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
            Profesional{' '}
            {!isEditMode && !roundRobinHabilitado && (
                ...
                render={({ field }) => (
                  <MultiSelect
                    ...
                    helper={
                      ...
                      watchServicios?.length > 0 &&
                      `${watchServicios.length} servicio(s) seleccionado(s)`
                    }
                  />
                )}
              />
