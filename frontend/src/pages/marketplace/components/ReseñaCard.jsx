 ...
 * />
 */
export const ReseñaCard = memo(function ReseñaCard({
  resena,
  canResponder = false,
  onResponder,
  className,
  ...
  const { shouldTruncate, comentarioMostrado } = useMemo(() => {
    const _shouldTruncate =
      resena.comentario && resena.comentario.length > COMENTARIO_LIMIT;
    const _comentarioMostrado =
      expanded || !_shouldTruncate
        ? resena.comentario
        : `${resena.comentario.substring(0, COMENTARIO_LIMIT)}...`;
    return {
      shouldTruncate: _shouldTruncate,
      comentarioMostrado: _comentarioMostrado,
    };
