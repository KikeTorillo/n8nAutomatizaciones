...
const ModuleGuard = memo(function ModuleGuard({
  ...
  const { tieneAlgunModulo, tieneTodosModulos, isLoading } = useModulos();
  ...
  if (isLoading) {
    return (
      <div className="flex items-center justify-center p-8">
        <div
          className={`animate-spin rounded-full h-8 w-8 border-b-2 ${SEMANTIC_COLORS.primary.borderSolid}`}
        ></div>
      </div>
    );
  }
