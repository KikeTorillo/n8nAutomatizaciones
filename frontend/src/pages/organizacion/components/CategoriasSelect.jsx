...
const COLORES_TIPO = {
  especialidad:
    'bg-secondary-100 dark:bg-secondary-900/40 text-secondary-700 dark:text-secondary-300 border-secondary-200 dark:border-secondary-700',
  nivel:
...
const CategoriasSelect = forwardRef(
  (
    ...
    // Fetch categorías
    const { data: categoriasAgrupadas = {}, isLoading: loadingAgrupadas } =
      useCategoriasAgrupadas();
    const { data: categoriasTodas = [], isLoading: loadingTodas } =
      useCategoriasProfesional(
    ...
    const categoriasMap = useMemo(() => {
      ...
      if (tipoFiltro && categoriasTodas.length > 0) {
        categoriasTodas.forEach((cat) =>
          map.set(cat.id, { ...cat, tipo_categoria: tipoFiltro })
        );
      } else {
        Object.entries(categoriasAgrupadas).forEach(([tipo, cats]) => {
          cats.forEach((cat) =>
            map.set(cat.id, { ...cat, tipo_categoria: tipo })
          );
        });
      }
    ...
    const baseStyles =
      'w-full min-h-[48px] px-4 py-2 border rounded-lg transition-colors focus:outline-none focus:ring-2 focus:ring-offset-0 disabled:opacity-50 disabled:cursor-not-allowed bg-white dark:bg-gray-800 cursor-pointer';
            ...
            <div className="flex items-center flex-wrap gap-1.5 min-h-[32px] pr-16">
              {isLoading ? (
                ...
                selectedCategorias.map((cat) => {
                  ...
                  const colorClase =
                    COLORES_TIPO[cat.tipo_categoria] || COLORES_TIPO.general;
