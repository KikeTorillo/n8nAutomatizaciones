...
// Helper para formatear precios (movido fuera del componente)
const formatearPrecio = (precio) => `$${parseFloat(precio || 0).toFixed(2)}`;
    ...
    <div className="p-4 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
      ...
      <div className="flex items-center gap-4">
        <div className="flex items-center gap-2">
          <button
            onClick={() => onCantidadChange(item.id, cantidad - 1)}
            disabled={cantidad <= 1}
      ...
      <div className="mt-2 flex items-center gap-2">
        ...
        <input
          type="number"
          value={descuentoItem}
          onChange={(e) => onDescuentoChange(item.id, e.target.value)}
 ...
 * Ene 2026: Optimizado con useMemo y useCallback
 */
export default function CarritoVenta({
  ...
  const { subtotal, montoDescuentoGlobal, descuentoCuponFinal, total, totalEquivalente } = useMemo(() => {
    ...
    const descGlobal = (sub * (parseFloat(descuentoGlobal) / 100));
    const descCupon = cuponActivo?.descuento_calculado || 0;
    const descCuponFinal = hayPromocionExclusiva ? 0 : descCupon;
    const tot = sub - descGlobal - descCuponFinal - descuentoPromociones;
    const totEquiv = tasaCambio ? tot * parseFloat(tasaCambio) : null;

    return {
      subtotal: sub,
      montoDescuentoGlobal: descGlobal,
      descuentoCuponFinal: descCuponFinal,
      total: tot,
      totalEquivalente: totEquiv,
    };
  }, [items, descuentoGlobal, cuponActivo, hayPromocionExclusiva, descuentoPromociones, tasaCambio]);

  // Memoizar handlers
  const handleCantidadChange = useCallback((itemId, nuevaCantidad) => {
  ...
  const handleDescuentoItemChange = useCallback((itemId, descuento) => {
  ...
  const formatearEquivalente = useCallback((monto) => {
    if (!monto) return null;
    return `≈ $${parseFloat(monto).toFixed(2)} ${monedaSecundaria}`;
  }, [monedaSecundaria]);

  // Memoizar IDs de productos para cupones
  const productosIds = useMemo(
    () => items.map(item => item.producto_id || item.id),
    [items]
  );
          ...
          <div className="divide-y divide-gray-200 dark:divide-gray-700">
            {items.map((item) => (
              <CarritoItem
                ...
                onDescuentoChange={handleDescuentoItemChange}
