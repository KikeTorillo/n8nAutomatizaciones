...
deploy_up() {
    ...
    docker compose -f docker-compose.prod.yml up -d || true  # Continuar aunque falle por timing

    log_info "Esperando que los servicios estén listos (35s)..."
    sleep 35

    # Verificar que frontend esté corriendo, si no levantarlo manualmente
    FRONTEND_STATUS=$(docker compose -f docker-compose.prod.yml ps frontend --format json 2>/dev/null | grep -o '"State":"[^"]*"' | cut -d'"' -f4)
