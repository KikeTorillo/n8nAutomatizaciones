...
function TipoIncapacidadInfo({ tipo }) {
  ...
  return (
    <div className={`rounded-lg p-4 ${config.bgColor}`}>
      <h4 className={`font-medium ${config.textColor} mb-2`}>{config.label}</h4>
...
function IncapacidadFormModal({ isOpen, onClose, profesionalInicial = null }) {
  ...
  // Opciones de tipos
  const opcionesTipos = Object.entries(TIPOS_INCAPACIDAD_CONFIG).map(
    ([key, config]) => ({
      value: key,
      label: config.label,
    })
    ...
    >
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="text-sm text-amber-800 dark:text-amber-200">
              ...
              <p className="text-amber-700 dark:text-amber-300">
                Las incapacidades generan un bloqueo automático en el calendario
                del profesional. El estado del profesional cambiará a
                &quot;Incapacidad&quot; mientras esté vigente.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-3">
          ...
          {tipoIncapacidad && <TipoIncapacidadInfo tipo={tipoIncapacidad} />}
        </div>
          ...
          <FormGroup
            ...
            helper={
              tipoIncapacidad === 'maternidad'
                ? 'Calculada automaticamente (84 dias)'
                : undefined
            }
          >
            <Input
        ...
        <FormGroup
          ...
          helper={
            diasCalculados > 0
              ? `Calculado: ${diasCalculados} dias segun las fechas`
              : undefined
          }
        >
          <Input
