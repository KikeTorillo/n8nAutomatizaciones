...
function ConfigurarChatbotModal({ isOpen, onClose, onSuccess }) {
    ...
    try {
      ...
      toast.error(
        error.response?.data?.error ||
          error.message ||
          'Error al configurar el bot de Telegram'
      );
    }
  };
  ...
  const handleWhatsAppSubmit = async (data) => {
    try {
      ...
      toast.error(
        error.response?.data?.error ||
          error.message ||
          'Error al configurar el bot de WhatsApp'
      );
    }
  };
  ...
  return (
    <Drawer
      ...
      title={
        selectedPlatform
          ? `Configurar ${selectedPlatform === 'telegram' ? 'Telegram' : 'WhatsApp'}`
          ...
          <>
            <div className="text-center mb-6">
              ...
              <p className="text-gray-600 dark:text-gray-400">
                Selecciona la plataforma donde quieres conectar tu chatbot
                inteligente
              </p>
            </div>
