    ...
    try {
      ...
      const dataExport = {
        ...
        resumen: {
          ...
          total_monto: comisiones.reduce(
            (sum, c) => sum + parseFloat(c.monto_comision),
            0
          ),
          total_pendientes: comisiones.filter(
            (c) => c.estado_pago === 'pendiente'
          ).length,
          total_pagadas: comisiones.filter((c) => c.estado_pago === 'pagada')
            .length,
        },
