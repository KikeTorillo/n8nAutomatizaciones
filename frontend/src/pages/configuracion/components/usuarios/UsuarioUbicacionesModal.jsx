 ...
 */
function UsuarioUbicacionesModal({ isOpen, onClose, usuarioId }) {
  ...
  const {
    ...
    isLoading,
    refetch,
  } = useUbicacionesDisponiblesUsuario(usuarioId);
  ...
  const toggleSeleccion = (ubicacionId) => {
    setSeleccionados((prev) => {
      ...
      if (!nuevoEstado[ubicacionId]) {
        setPermisos((p) => {
          const { [ubicacionId]: _removed, ...rest } = p;
          return rest;
        });
      } else {
      ...
      <div className="space-y-4">
        ...
        {ubicacionesSeleccionadas > 0 && (
          <div className="bg-primary-50 dark:bg-primary-900/30 text-primary-700 dark:text-primary-300 text-sm px-4 py-2 rounded-lg">
            {ubicacionesSeleccionadas} ubicación
            {ubicacionesSeleccionadas > 1 ? 'es' : ''} seleccionada
                ...
                <>
                  ...
                  <div className="flex items-start gap-2 text-left bg-amber-50 dark:bg-amber-900/20 p-3 rounded-lg mt-4">
                    ...
                    <p className="text-sm text-amber-800 dark:text-amber-300">
                      El usuario debe estar asignado a una sucursal que tenga
                      ubicaciones de almacén configuradas. Todas las ubicaciones
                      disponibles ya están asignadas, o las sucursales del
                      usuario no tienen ubicaciones.
                    </p>
                  </div>
                </>
              ) : (
                          ...
                          >
                            ...
                            <div className="flex flex-wrap gap-2">
                              <button
                                type="button"
                                onClick={() =>
                                  togglePermiso(ubicacion.id, 'es_default')
                                }
                              ...
                              <button
                                type="button"
                                onClick={() =>
                                  togglePermiso(ubicacion.id, 'puede_recibir')
                                }
                                className={`flex items-center gap-1 px-2 py-1 rounded text-xs font-medium transition-colors ${
                              ...
                              <button
                                type="button"
                                onClick={() =>
                                  togglePermiso(ubicacion.id, 'puede_despachar')
                                }
                                className={`flex items-center gap-1 px-2 py-1 rounded text-xs font-medium transition-colors ${
        ...
        <div className="flex gap-3 pt-4 border-t border-gray-200 dark:border-gray-700">
          ...
          <Button
            ...
            disabled={
              ubicacionesSeleccionadas === 0 || asignarMutation.isPending
            }
            isLoading={asignarMutation.isPending}
            className="flex-1"
          >
            Asignar{' '}
            {ubicacionesSeleccionadas > 0 && `(${ubicacionesSeleccionadas})`}
          </Button>
        </div>
      </div>
    </Drawer>
