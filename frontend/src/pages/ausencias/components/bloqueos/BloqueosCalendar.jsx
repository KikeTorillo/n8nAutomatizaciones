...
import CalendarioHeader from '@/pages/citas/components/CalendarioHeader';
...
function BloqueosCalendar({
  ...
  const bloqueosPorFecha = useMemo(() => {
    ...
    bloqueos.forEach((bloqueo) => {
      ...
      const fechasBloqueo = generarRangoFechas(
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden">
      ...
      <div className={isMobile ? 'p-2' : 'p-4'}>
        ...
        <div
          className={`grid grid-cols-7 ${isMobile ? 'gap-1 mb-1' : 'gap-1 mb-2'}`}
        >
          ...
          {diasDelCalendario.map((dia, index) => {
            ...
            return (
              <CalendarioDiaBloqueo
                ...
                onCrearBloqueo={
                  onCrearBloqueo ? () => onCrearBloqueo(fechaISO) : undefined
                }
                isLoading={isLoading}
        ...
        <div
          className={`${isMobile ? 'mt-3 pt-3' : 'mt-6 pt-4'} border-t border-gray-200 dark:border-gray-700`}
        >
          <p
            className={`font-semibold text-gray-600 dark:text-gray-400 mb-2 ${isMobile ? 'text-[10px]' : 'text-xs'}`}
          >
            Tipos de bloqueo:
          </p>
          <div className={`flex flex-wrap ${isMobile ? 'gap-2' : 'gap-3'}`}>
            <div className="flex items-center gap-1.5">
              <div
                className={`${isMobile ? 'w-2 h-2' : 'w-3 h-3'} rounded-full bg-secondary-500`}
              ></div>
              <span
                className={`text-gray-600 dark:text-gray-400 ${isMobile ? 'text-[10px]' : 'text-xs'}`}
              >
                {LABELS_TIPO_BLOQUEO.vacaciones}
              </span>
            </div>
            <div className="flex items-center gap-1.5">
              <div
              ...
              <span
                className={`text-gray-600 dark:text-gray-400 ${isMobile ? 'text-[10px]' : 'text-xs'}`}
              >
                {LABELS_TIPO_BLOQUEO.feriado}
              </span>
            </div>
            <div className="flex items-center gap-1.5">
              <div
                className={`${isMobile ? 'w-2 h-2' : 'w-3 h-3'} rounded-full bg-orange-500`}
              ></div>
              <span
                className={`text-gray-600 dark:text-gray-400 ${isMobile ? 'text-[10px]' : 'text-xs'}`}
              >
                {LABELS_TIPO_BLOQUEO.mantenimiento}
              </span>
            </div>
            {!isMobile && (
              <>
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="text-xs text-gray-600 dark:text-gray-400">
                    {LABELS_TIPO_BLOQUEO.evento_especial}
                  </span>
                </div>
                <div className="flex items-center gap-1.5">
                  <div className="w-3 h-3 rounded-full bg-amber-500"></div>
                  <span className="text-xs text-gray-600 dark:text-gray-400">
                    {LABELS_TIPO_BLOQUEO.emergencia}
                  </span>
                </div>
                <div className="flex items-center gap-1.5">
                  <div className="w-3 h-3 rounded-full bg-primary-500"></div>
                  <span className="text-xs text-gray-600 dark:text-gray-400">
                    {LABELS_TIPO_BLOQUEO.personal}
                  </span>
                </div>
                <div className="flex items-center gap-1.5">
                  <div className="w-3 h-3 rounded-full bg-gray-500"></div>
                  <span className="text-xs text-gray-600 dark:text-gray-400">
                    {LABELS_TIPO_BLOQUEO.organizacional}
                  </span>
                </div>
              </>
            )}
            ...
            <div className="flex items-start gap-2">
              <div className="text-primary-600 dark:text-primary-300 text-sm">
                ...
                <ul className="list-disc list-inside text-xs space-y-0.5 text-primary-700 dark:text-primary-300">
                  <li>
                    Los días con fondo rojo tienen bloqueos organizacionales
                    (afectan a todos)
                  </li>
                  <li>
                    El icono de reloj indica que el bloqueo es de horario
                    parcial (no todo el día)
                  </li>
                  ...
                  {!profesionalId && (
                    <li>
                      Mostrando todos los bloqueos (organizacionales e
                      individuales)
                    </li>
                  )}
                </ul>
