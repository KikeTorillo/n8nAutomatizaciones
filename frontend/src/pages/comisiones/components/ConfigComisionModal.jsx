...
function ConfigComisionModal({
  ...
  const { data: profesionalesData, isLoading: loadingProfesionales } =
    useProfesionales();
  ...
  const { data: categoriasData, isLoading: loadingCategorias } =
    useCategorias();

  // Extraer arrays de las respuestas (hooks con transformList retornan {items, paginacion})
  const profesionales =
    profesionalesData?.items || profesionalesData?.profesionales || [];
  ...
  useEffect(() => {
    if (isEditMode && configuracion && isOpen) {
      reset({
        ...
        servicio_id: configuracion.servicio_id
          ? String(configuracion.servicio_id)
          : '',
        producto_id: configuracion.producto_id
          ? String(configuracion.producto_id)
          : '',
        categoria_producto_id: configuracion.categoria_producto_id
          ? String(configuracion.categoria_producto_id)
          : '',
  ...
  const onSubmit = async (data) => {
    try {
      ...
      if (aplicaA === 'servicio') {
        payload.servicio_id =
          data.servicio_id && data.servicio_id !== ''
        ...
        if (!payload.producto_id) {
          payload.categoria_producto_id =
            data.categoria_producto_id && data.categoria_producto_id !== ''
  ...
  return (
    <Drawer
      ...
      subtitle={
        isEditMode
          ? 'Modifica la configuración de comisión'
          : 'Define el esquema de comisión para el profesional'
      }
    >
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        ...
        <Controller
          ...
          render={({ field }) => (
            <FormGroup
              label={
                tipoComision === 'porcentaje' ? 'Porcentaje' : 'Monto Fijo'
              }
