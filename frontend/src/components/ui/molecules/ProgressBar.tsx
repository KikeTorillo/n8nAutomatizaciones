...
export const ProgressBar = memo(
  forwardRef<HTMLDivElement, ProgressBarProps>(function ProgressBar(
    ...
    const percentage =
      percentageProp ?? (max > 0 ? Math.round((value / max) * 100) : 0);
    ...
    return (
      <div ref={ref} className={cn('flex items-center gap-3', className)}>
        {label && (
          <span
            className={cn('text-sm font-medium flex-shrink-0', textColorClass)}
          >
            {label}
          </span>
        )}
 ...
 */
export const LimitProgressBar = memo(function LimitProgressBar({
  label,
  usado,
  limite,
  porcentaje,
