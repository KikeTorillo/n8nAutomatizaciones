...
import { Plus, X, Search, Users, UserPlus, Loader2 } from 'lucide-react';
...
export default function ListaClientesView({ listaId }) {
  ...
  const asignarMutation = useMutation({
    mutationFn: async (clienteId) => {
      return await clientesApi.actualizar(clienteId, {
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        <p className="text-sm text-gray-500 dark:text-gray-400">
          {clientes.length} cliente{clientes.length !== 1 ? 's' : ''} asignado
          {clientes.length !== 1 ? 's' : ''}
        </p>
        <Button
          size="sm"
