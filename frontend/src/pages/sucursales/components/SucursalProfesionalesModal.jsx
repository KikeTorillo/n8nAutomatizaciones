...
function SucursalProfesionalesModal({
  ...
  const { data: profesionalesData, isLoading } = useProfesionales({
    activo: true,
  });
  ...
  const idsAsignados = (profesionalesAsignados || []).map(
    (p) => p.profesional_id || p.id
  );
  ...
  const profesionalesSeleccionados =
      ...
      <div className="space-y-4">
        ...
        {profesionalesSeleccionados > 0 && (
          <div className="bg-primary-50 dark:bg-primary-900/30 text-primary-700 dark:text-primary-300 text-sm px-4 py-2 rounded-lg">
            {profesionalesSeleccionados} profesional
            {profesionalesSeleccionados > 1 ? 'es' : ''} seleccionado
            {profesionalesSeleccionados > 1 ? 's' : ''}
          </div>
        )}
        ...
        <div className="flex gap-3 pt-4 border-t border-gray-200 dark:border-gray-700">
          ...
          <Button
            ...
            disabled={
              profesionalesSeleccionados === 0 || asignarMutation.isPending
            }
