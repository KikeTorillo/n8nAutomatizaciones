...
const servicioCreateSchema = z.object({
  ...
  requiere_preparacion_minutos: z
    .number()
    .int('Debe ser un número entero')
...
function ServicioFormDrawer({
  ...
  const monedasDisponibles = todasLasMonedas.filter(
    (m) => m.codigo !== monedaOrg
  );

  // Fetch datos del servicio en modo edición
  const { data: servicioData, isLoading: loadingServicio } = useServicio(
    servicioId,
    {
  ...
  const {
    ...
    resolver: zodResolver(
      isEditMode ? servicioEditSchema : servicioCreateSchema
    ),
  ...
  useEffect(() => {
    if (isEditMode && servicioData && isOpen) {
      ...
      if (
        servicioData.precios_moneda &&
        servicioData.precios_moneda.length > 0
      ) {
        setPreciosMoneda(
          servicioData.precios_moneda.map((p) => ({
            moneda: p.moneda,
            precio: p.precio || '',
  ...
  const agregarPrecioMoneda = () => {
    ...
    const monedaDisponible = monedasDisponibles.find(
      (m) => !monedasUsadas.includes(m.codigo)
    );
  ...
  const onSubmit = async (data) => {
    try {
      ...
      toast.error(
        error.message ||
          `Error al ${isEditMode ? 'actualizar' : 'crear'} servicio`
      );
    }
  };
  ...
  const isSubmittingAny =
    crearMutation.isPending || actualizarMutation.isPending || imagen.isPending;
          ...
          <div className="space-y-4">
                        ...
                        <Input
                          ...
                          onChange={(e) => {
                            ...
                            const newHoras =
                              value === ''
                                ? 0
                                : Math.min(Math.max(parseInt(value), 0), 8);
                        ...
                        <Input
                          ...
                          onChange={(e) => {
                            ...
                            const newMinutos =
                              value === ''
                                ? 0
                                : Math.min(Math.max(parseInt(value), 0), 59);
            ...
            <div className="space-y-3">
              ...
              <p className="text-xs text-gray-500 dark:text-gray-400 -mt-1">
                Tiempo adicional antes y después del servicio para preparación
                del espacio
              </p>
                      ...
                      <Controller
                        ...
                        render={({ field: limpiezaField }) => {
                          const tiempoTotal =
                            (prepField.value || 0) +
                            duracionField.value +
                            (limpiezaField.value || 0);
                          return (
                            <div className="p-3 bg-primary-50 dark:bg-primary-900/20 rounded-lg">
                              <p className="text-sm text-primary-700 dark:text-primary-300">
                                <span className="font-medium">
                                  Tiempo total por cita:
                                </span>{' '}
                                ...
                                {(prepField.value > 0 ||
                                  ...
                                  <span className="text-xs text-primary-600 dark:text-primary-400 ml-2">
                                    ({prepField.value || 0} prep +{' '}
                                    {duracionField.value} servicio +{' '}
                                    {limpiezaField.value || 0} limpieza)
                                  </span>
                                )}
                              </p>
                            </div>
                ...
                {mostrarPreciosMoneda && (
                  <div className="p-4 space-y-4">
                    {preciosMoneda.map((precio, index) => {
                      const monedaInfo = monedasDisponibles.find(
                        (m) => m.codigo === precio.moneda
                      );
                      const monedasUsadas = preciosMoneda.map((p) => p.moneda);
                      const opcionesMoneda = monedasDisponibles.filter(
                        (m) =>
                          m.codigo === precio.moneda ||
                          !monedasUsadas.includes(m.codigo)
                      );
              ...
              <div>
                ...
                {loadingProfesionales ? (
                    ...
                    `}
                      >
                        ...
                        <div
                          ...
                          style={{
                            backgroundColor: prof.color_calendario || '#3B82F6',
                          }}
                        />
                  ...
                  <div className="text-sm text-gray-600 dark:text-gray-400 bg-primary-50 dark:bg-primary-900/30 border border-primary-200 dark:border-primary-800 rounded-lg p-3">
                    💡 No hay profesionales disponibles. Puedes crear el
                    servicio ahora y asignar profesionales después desde la
                    página de <strong>Profesionales → Servicios</strong>.
                  </div>
                )}
                ...
                {selectedProfessionals.length > 0 && (
                  <p className="mt-2 text-sm text-gray-600 dark:text-gray-400">
                    {selectedProfessionals.length} profesional(es)
                    seleccionado(s)
                  </p>
                )}
              </div>
