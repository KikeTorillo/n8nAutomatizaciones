...
const hooks = createCRUDHooks<Servicio, CrearServicioData, ActualizarServicioData>({
  name: 'servicio',
...
export function useProfesionalesServicio(servicioId: number | null | undefined): UseQueryResult<Profesional[]> {
  return useQuery({
...
export function useAsignarProfesional(): UseMutationResult<Profesional, Error, AsignarProfesionalVariables> {
  const queryClient = useQueryClient();

  return useMutation({
    mutationFn: async ({ servicioId, profesionalId, configuracion = {} }: AsignarProfesionalVariables) => {
...
export function useDesasignarProfesional(): UseMutationResult<DesasignarProfesionalVariables, Error, DesasignarProfesionalVariables> {
  const queryClient = useQueryClient();

  return useMutation({
    mutationFn: async ({ servicioId, profesionalId }: DesasignarProfesionalVariables) => {
