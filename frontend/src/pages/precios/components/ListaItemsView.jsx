...
export default function ListaItemsView({ listaId }) {
  ...
  const { data: productosEncontrados = [], isLoading: buscandoProductos } =
    useQuery({
      ...
      queryFn: async () => {
        ...
        const response = await inventarioApi.buscarProductos({
          q: busquedaProducto,
          limit: 10,
        });
        return response.data.data || [];
  ...
  const { data: categoriasEncontradas = [], isLoading: buscandoCategorias } =
    useQuery({
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        <p className="text-sm text-gray-500 dark:text-gray-400">
          {items.length} item{items.length !== 1 ? 's' : ''} configurado
          {items.length !== 1 ? 's' : ''}
        </p>
        <Button
          size="sm"
          ...
          <div className="space-y-2">
            ...
            <p className="text-xs text-gray-500 dark:text-gray-400">
              {tipoAplicacion === 'producto' &&
                'El precio/descuento aplica solo a este producto'}
              {tipoAplicacion === 'categoria' &&
                'El precio/descuento aplica a todos los productos de la categoria'}
              {tipoAplicacion === 'global' &&
                'El precio/descuento aplica a todos los productos sin regla especifica'}
            </p>
          </div>
          ...
          {tipoAplicacion === 'producto' && productoSeleccionado && (
            <div className="flex items-center justify-between bg-white dark:bg-gray-800 p-3 rounded-lg border border-gray-200 dark:border-gray-700">
              <div>
                ...
                <div className="text-xs text-gray-500 dark:text-gray-400">
                  SKU: {productoSeleccionado.sku} | Precio base: $
                  {parseFloat(productoSeleccionado.precio_venta || 0).toFixed(
                    2
                  )}
                </div>
              </div>
          ...
          {tipoAplicacion === 'categoria' && categoriaSeleccionada && (
            <div className="flex items-center justify-between bg-white dark:bg-gray-800 p-3 rounded-lg border border-gray-200 dark:border-gray-700">
              <div>
                ...
                <div className="text-xs text-gray-500 dark:text-gray-400">
                  {categoriaSeleccionada.total_productos || 0} productos en esta
                  categoria
                </div>
              </div>
              <button
          ...
          {tipoAplicacion === 'global' && (
            <div className="bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-lg p-3">
              <p className="text-sm text-yellow-800 dark:text-yellow-200">
                Esta regla aplicara a todos los productos que no tengan una
                regla especifica (producto o categoria).
              </p>
            </div>
          )}
                  ...
                  <td className="py-3">
                    {item.precio_fijo ? (
                      <span className="text-gray-900 dark:text-gray-100">
                        $
                        {item.precio_fijo.toLocaleString('es-MX', {
      ...
      <ConfirmDialog
        ...
        onConfirm={() =>
          eliminarMutation.mutate(getItemModalData('deleteItem')?.id)
        }
        title="Eliminar Item"
