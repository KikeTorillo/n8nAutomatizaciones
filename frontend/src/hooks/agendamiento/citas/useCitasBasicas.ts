...
import type { Cita, CitasListParams, CitasListResponse } from '@/types/entities/cita';

/**
 * Hook para listar citas con filtros y paginación
 * @param params - Filtros: { fecha_desde, fecha_hasta, profesional_id, estado, cliente_id, servicio_id, page, limit }
 * @returns { data: { citas, meta }, isLoading, error, refetch }
 */
export function useCitas(params: CitasListParams = {}) {
  return useQuery({
    queryKey: queryKeys.agendamiento.citas.list(params),
    queryFn: async (): Promise<CitasListResponse> => {
 ...
 */
export function useCita(id: number | undefined) {
  return useQuery({
