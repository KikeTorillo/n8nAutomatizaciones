...
function ComisionesDashboard() {
  ...
  const { data: graficaData, isLoading: loadingGrafica } =
    useGraficaComisionesPorDia({
      fecha_desde: desde,
      fecha_hasta: hasta,
      profesional_id: profesionalSeleccionado || undefined,
  ...
  const comisionesPendientes = parseFloat(
    dashboard?.resumen?.monto_pendiente || 0
  );
  const comisionesPagadas = parseFloat(dashboard?.resumen?.monto_pagado || 0);
  const totalProfesionales = parseInt(
    dashboard?.resumen?.profesionales_activos || 0
  );
  ...
  const barChartDataPorDia = {
    labels:
      graficaData?.map((d) =>
        format(new Date(d.fecha), 'dd MMM', { locale: es })
      ) || [],
    datasets: [
      {
        label: 'Comisiones',
