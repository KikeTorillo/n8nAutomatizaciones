...
const BloqueosDiaDrawer = memo(function BloqueosDiaDrawer({
  ...
  const fechaFormateada = format(new Date(fecha), "EEEE d 'de' MMMM", {
    locale: es,
  });
      ...
      {bloqueos.length === 0 ? (
        ...
        <div className="space-y-3">
          {bloqueos.map((bloqueo) => {
            ...
            const tipoLabel =
              LABELS_TIPO_BLOQUEO[bloqueo.tipo_bloqueo_codigo] ||
              bloqueo.tipo_bloqueo_codigo;
              ...
              >
                ...
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-2">
                    {/* Icono de tipo */}
                    <div
                      className={`w-8 h-8 rounded-full ${colores.bg} flex items-center justify-center`}
                    >
                      {esOrganizacional ? (
                ...
                <div className="flex items-center justify-between text-sm">
                  <span className="text-gray-500 dark:text-gray-400">
                    {esOrganizacional
                      ? 'Afecta a todos'
                      : bloqueo.profesional_nombre || 'Individual'}
                  </span>
                  {!esDiaCompleto && (
