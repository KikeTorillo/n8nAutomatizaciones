      ...
      <div className="p-4 border-b border-gray-100 dark:border-gray-700">
        <div className="flex items-start justify-between">
          <div className="flex items-start gap-3 flex-1">
            {/* Ícono */}
            <div
              className={`relative w-12 h-12 ${colores.badge} rounded-lg flex items-center justify-center flex-shrink-0`}
            >
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex flex-wrap items-center gap-2 mt-1">
                <span
                  className={`px-2 py-1 text-xs font-medium rounded ${colores.bg} ${colores.text}`}
                >
                  {obtenerLabelTipoBloqueo(bloqueo.tipo_bloqueo_codigo)}
                </span>
                {esAutoGenerado && (
                  <span
                    className={`px-2 py-1 text-xs font-medium rounded ${obtenerColorOrigenBloqueo(bloqueo.origen_bloqueo)}`}
                  >
  ...
  return (
    <div>
      <div className="mb-4 text-sm text-gray-600 dark:text-gray-400">
        Mostrando {bloqueos.length}{' '}
        {bloqueos.length === 1 ? 'bloqueo' : 'bloqueos'}
      </div>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
