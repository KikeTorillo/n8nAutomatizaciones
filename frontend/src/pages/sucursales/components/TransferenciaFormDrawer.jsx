...
import { useSucursales, useCrearTransferencia } from '@/hooks/sistema';
...
const transferenciaSchema = z
  .object({
    sucursal_origen_id: z.number({
      required_error: 'Selecciona la sucursal de origen',
    }),
    sucursal_destino_id: z.number({
      required_error: 'Selecciona la sucursal de destino',
    }),
...
function TransferenciaFormDrawer({ isOpen, onClose }) {
  ...
  const onSubmit = async (data) => {
    ...
    const itemsInvalidos = items.filter(
      (i) => i.cantidad_enviada > i.stock_actual
    );
    if (itemsInvalidos.length > 0) {
      toast.error(
        `Stock insuficiente para: ${itemsInvalidos.map((i) => i.nombre).join(', ')}`
      );
      return;
    }
