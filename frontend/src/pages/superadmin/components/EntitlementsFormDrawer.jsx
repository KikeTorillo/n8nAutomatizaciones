 ...
 */
function EntitlementsFormDrawer({
  isOpen,
  onClose,
  plan,
  modulosDisponibles = [],
  ...
  const formatModuloName = (modulo) => {
    return modulo.replace(/-/g, ' ').replace(/\b\w/g, (l) => l.toUpperCase());
  };
  ...
  const onSubmit = (data) => {
    ...
    mutation.mutate(
      ...
      },
      {
        ...
        onError: (err) =>
          showError(err.message || 'Error al actualizar entitlements'),
      }
    );
  };
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        ...
        <div className="pt-4 border-t border-gray-200 dark:border-gray-700">
          <CheckboxField
            ...
            onChange={(e) =>
              setValue('sincronizar_organizaciones', e.target.checked)
            }
          />
        </div>
