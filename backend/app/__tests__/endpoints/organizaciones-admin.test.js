...
describe('Endpoints Administrativos de Organizaciones', () => {
  ...
  // ============================================================================
  // Tests de ONBOARDING - Flujo Completo de Alta
  // ============================================================================

  describe('POST /api/v1/organizaciones/onboarding - Alta Completa', () => {
    test('Onboarding completo: org + admin + plantilla servicios', async () => {
      const uniqueId = getUniqueTestId();

      const onboardingData = {
        // Datos organización
        organizacion: {
          nombre_comercial: `Nueva Empresa ${uniqueId}`,
          ...
          email_contacto: `contacto-${uniqueId}@test.com`
        },
        // Datos admin inicial
        admin: {
          nombre: 'Admin',
        ...
        enviar_email_bienvenida: false // En tests no enviar emails
      };
      ...
      expect(response.body.data.organizacion.nombre_comercial).toBe(onboardingData.organizacion.nombre_comercial);
      expect(response.body.data.admin.email).toBe(onboardingData.admin.email);
      expect(response.body.data.admin).toHaveProperty('token'); // JWT para login inmediato

      // Si aplicó plantilla, debe tener servicios
      if (onboardingData.aplicar_plantilla_servicios) {
        expect(response.body.data).toHaveProperty('servicios_creados');
        expect(response.body.data.servicios_creados).toBeGreaterThan(0);
      }
    });

    test('Onboarding sin plantilla de servicios', async () => {
      const uniqueId = getUniqueTestId();

      const onboardingData = {
        organizacion: {
          nombre_comercial: `Empresa Sin Plantilla ${uniqueId}`,
          razon_social: `Razón ${uniqueId}`,
          tipo_industria: 'spa',
          plan: 'profesional'
        },
        admin: {
    ...
    test('Falla con email de admin duplicado', async () => {
      const uniqueId = getUniqueTestId();

      const onboardingData = {
        organizacion: {
          nombre_comercial: `Empresa Duplicada ${uniqueId}`,
          razon_social: `Razón ${uniqueId}`,
          tipo_industria: 'barberia',
          plan: 'basico'
        },
        admin: {
          nombre: 'Admin',
          apellidos: 'Duplicado',
          email: testAdmin.email, // Email ya existente
          password: 'SecurePassword123!'
        }
      };
    ...
    test('Falla sin datos de organización', async () => {
      const uniqueId = getUniqueTestId();
    ...
    test('Falla sin datos de admin', async () => {
      const uniqueId = getUniqueTestId();
      ...
      const response = await request(app)
        ...
        .send({
          organizacion: {
            nombre_comercial: `Empresa ${uniqueId}`,
            tipo_industria: 'barberia',
            plan: 'basico'
          }
        })
    ...
    test('Falla con contraseña débil de admin', async () => {
      const uniqueId = getUniqueTestId();
    ...
    test('Falla sin autenticación', async () => {
      const response = await request(app)
        .post('/api/v1/organizaciones/onboarding')
        .send({
          organizacion: { nombre_comercial: 'Test' },
          admin: { email: 'test@test.com', password: 'Pass123!' }
        })
        .expect(401);
