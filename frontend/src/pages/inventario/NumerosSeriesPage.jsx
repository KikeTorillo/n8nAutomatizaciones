...
const getEstadoLabel = (estado) => ESTADOS_NUMERO_SERIE[estado]?.label || estado;
...
/**
 * Acciones por fila para la tabla
 */
function RowActions({ row, onVerDetalle, onVerHistorial, onMarcarDefectuoso }) {
  return (
    <DataTableActions>
      <DataTableActionButton
        icon={Eye}
      ...
      {row.estado === 'disponible' && (
        <DataTableActionButton
          ...
          onClick={() => onMarcarDefectuoso(row.id)}
          variant="danger"
        />
      )}
    </DataTableActions>
...
/**
 * Estadísticas de números de serie
 */
function StatsSection({ estadisticas }) {
 ...
 */
function AlertaProximosVencer({ proximosVencer }) {
  if (!proximosVencer || proximosVencer.length === 0) return null;
...
/**
 * Modal de detalle de número de serie
 */
function DetalleModal({ isOpen, onClose, detalle }) {
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={onClose}
      title="Detalle del Número de Serie"
      size="lg"
    >
      <div className="space-y-4">
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="text-sm text-gray-500 dark:text-gray-400">Número de Serie</label>
            <p className="font-mono font-medium">{detalle.numero_serie}</p>
          </div>
          <div>
            <label className="text-sm text-gray-500 dark:text-gray-400">Estado</label>
            <p>
              <Badge variant={ESTADO_NS_VARIANT[detalle.estado] || 'default'} size="sm">
          ...
          <div>
            ...
            <p className="text-xs text-gray-500">{detalle.producto_sku}</p>
          </div>
          <div>
        ...
        {detalle.notas && (
          <div className="border-t dark:border-gray-700 pt-4">
            ...
            <p className="text-sm text-gray-600 dark:text-gray-300">{detalle.notas}</p>
          </div>
        )}
      </div>
    </Modal>
 ...
 */
function HistorialModal({ isOpen, onClose, historial }) {
  return (
    <Modal
      isOpen={isOpen}
      onClose={onClose}
...
function NumerosSeriesPage() {
  ...
  const { filtros, filtrosQuery, setFiltros, limpiarFiltros } = useFilters(
    INITIAL_FILTERS,
    { moduloId: 'inventario.numeros-serie', debounceFields: ['busqueda'] }
  );
  ...
  // Queries
  const { data: numerosSerieData, isLoading, refetch } = useNumerosSerie(filtrosQuery);
  ...
  // Queries de detalle (basadas en modal activo)
  ...
  const handleVerDetalle = useCallback((id) => openModal('detalle', { id }), [openModal]);
  const handleVerHistorial = useCallback((id) => openModal('historial', { id }), [openModal]);
  const handleAbrirDefectuoso = useCallback((id) => openModal('defectuoso', { id }), [openModal]);
    ...
    ...COLUMNS,
    {
      ...
      render: (row) => (
        <RowActions
          ...
          onMarcarDefectuoso={handleAbrirDefectuoso}
        />
      ),
    },
  ], [handleVerDetalle, handleVerHistorial, handleAbrirDefectuoso]);
  ...
  return (
    ...
    >
      {/* Estadísticas */}
      <StatsSection estadisticas={estadisticas} />

      {/* Alerta de próximos a vencer */}
      <AlertaProximosVencer proximosVencer={proximosVencer} />
