...
function CancelarCitaModal({ isOpen, onClose, cita = null }) {
  ...
  const handleCancelar = async () => {
    ...
    try {
      ...
      toast.error(
        'Error al cancelar: ' +
          (error.response?.data?.message || error.message || 'Intente de nuevo')
      );
    }
  };
        ...
        <div className="bg-red-50 dark:bg-red-900/30 border border-red-200 dark:border-red-800 rounded-lg p-4">
          <div className="flex items-start gap-2">
            ...
            <p className="text-sm text-red-900 dark:text-red-200">
              <strong>Importante:</strong> Al cancelar esta cita, el horario
              quedará disponible para otras reservas. El cliente será notificado
              automáticamente si tiene configuradas las notificaciones.
            </p>
          </div>
        </div>
