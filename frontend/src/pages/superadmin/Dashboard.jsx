...
import OrganizacionDrawer from '@/pages/superadmin/components/OrganizacionDrawer';
...
export default function SuperAdminDashboard() {
  ...
  const { data: metricasSaaS, isLoading: isLoadingMetricas } =
    useMetricasSaaS();
  ...
  const confirmarSuspension = useCallback(() => {
    if (
      !confirmSuspender.data ||
      !motivoSuspension ||
      motivoSuspension.length < 10
    ) {
  ...
  if (errorDashboard) {
    return (
      <div className="bg-red-50 dark:bg-red-900/30 border-2 border-red-200 dark:border-red-800 rounded-lg p-6">
        <h3 className="text-red-800 dark:text-red-300 font-bold text-lg mb-2">
  ...
  const enTrial =
    metricasSaaS?.distribucion_por_estado?.find((d) => d.estado === 'trial')
      ?.cantidad || 0;
  ...
  return (
    <div className="space-y-6">
      ...
      <OrganizacionDrawer
        ...
        isLoadingAction={
          suspenderOrganizacion.isPending || reactivarOrganizacion.isPending
        }
      />
        ...
        message={
          <>
            ...
            <p className="mt-2 text-sm text-gray-500 dark:text-gray-400">
              Los usuarios de esta organización podrán acceder nuevamente al
              sistema.
            </p>
          </>
        }
