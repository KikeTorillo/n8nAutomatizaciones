...
function NotificacionesPreferencias() {
  ...
  // Queries
  const { data: preferenciasData = {}, isLoading: loadingPrefs } =
    useNotificacionesPreferencias();
  const { data: tiposData = {}, isLoading: loadingTipos } =
    useNotificacionesTipos();
  ...
  useEffect(() => {
    if (Object.keys(preferenciasData).length > 0) {
      ...
      const prefs = {};
      Object.values(preferenciasData)
        .flat()
        .forEach((pref) => {
          prefs[pref.tipo] = {
            in_app: pref.in_app,
  ...
  const handleToggleCategoria = (categoria, canal) => {
    ...
    const todosActivos = tipos.every(
      (t) => preferenciasLocales[t.tipo]?.[canal]
    );
  ...
  const handleGuardar = async () => {
    ...
    const preferencias = Object.entries(preferenciasLocales).map(
      ([tipo, canales]) => ({
        tipo,
  ...
  const categoriasConTipos = NOTIFICACION_CATEGORIAS.filter(
    (cat) => tiposData[cat.value]?.length > 0
  );
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        {hasChanges && (
          <Button onClick={handleGuardar} isLoading={actualizarPrefs.isPending}>
      ...
      <div className="space-y-6">
        {categoriasConTipos.map((categoria) => {
          ...
          const todosInApp = tipos.every(
            (t) => preferenciasLocales[t.tipo]?.in_app !== false
          );
          const todosEmail = tipos.every(
            (t) => preferenciasLocales[t.tipo]?.email === true
          );
                  ...
                  <div className="flex items-center gap-4">
                    <label className="flex items-center gap-2 cursor-pointer">
                      <input
                        ...
                        checked={todosInApp}
                        onChange={() =>
                          handleToggleCategoria(categoria.value, 'in_app')
                        }
                    ...
                    <label className="flex items-center gap-2 cursor-pointer">
                      <input
                        ...
                        checked={todosEmail}
                        onChange={() =>
                          handleToggleCategoria(categoria.value, 'email')
                        }
                        className="w-4 h-4 text-primary-600 border-gray-300 rounded focus:ring-primary-500"
                      />
