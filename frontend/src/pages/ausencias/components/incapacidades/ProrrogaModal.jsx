...
import { z } from 'zod';
import { ArrowUpRight, Calendar, FileText, Info } from 'lucide-react';
import { Button, Drawer, FormGroup, Input } from '@/components/ui';
import { useCrearProrroga, getTipoIncapacidadConfig } from '@/hooks/personas';

// Schema de validación para prórroga
const prorrogaSchema = z
  .object({
    folio_imss: z
      .string()
...
function ProrrogaModal({ isOpen, onClose, incapacidadOrigen }) {
  ...
  const tipoConfig = getTipoIncapacidadConfig(
    incapacidadOrigen.tipo_incapacidad
  );

  return (
    <Drawer isOpen={isOpen} onClose={handleClose} title="Crear Prórroga">
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        {/* Info de la incapacidad origen */}
        <div className={`rounded-lg p-4 ${tipoConfig.bgColor}`}>
          <div className="flex items-start gap-3">
            <ArrowUpRight
              className={`h-5 w-5 ${tipoConfig.textColor} flex-shrink-0 mt-0.5`}
            />
            <div className={tipoConfig.textColor}>
              ...
              <p className="text-sm opacity-80 mt-1">
                Período original: {formatFecha(incapacidadOrigen.fecha_inicio)}{' '}
                - {formatFecha(incapacidadOrigen.fecha_fin)}
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-lg p-4">
          <div className="flex items-start gap-3">
            ...
            <div className="text-sm text-amber-800 dark:text-amber-200">
              <p>
                La prórroga extenderá el bloqueo del profesional. Se requiere un
                nuevo folio IMSS.
              </p>
            </div>
          </div>
