...
const FAQ_ITEMS = [
  {
    ...
    respuesta:
      'Sí, puedes cambiar a un plan superior (upgrade) en cualquier momento y el cambio será inmediato. Para cambiar a un plan inferior (downgrade), contacta a nuestro equipo de soporte y el cambio se aplicará en tu próximo ciclo de facturación.',
  },
  {
    pregunta: '¿Qué métodos de pago aceptan?',
    respuesta:
      'Aceptamos tarjetas de crédito y débito (Visa, MasterCard, American Express), transferencias bancarias y pagos a través de MercadoPago. Los pagos se procesan de forma segura.',
  },
  {
    pregunta: '¿Qué pasa si cancelo mi suscripción?',
    respuesta:
      'Si cancelas, mantendrás acceso hasta el final de tu período de facturación actual. Después de eso, tu cuenta pasará a modo de solo lectura. Tus datos se conservarán por 90 días por si decides reactivar.',
  },
  {
    pregunta: '¿Ofrecen descuentos para pagos anuales?',
    respuesta:
      'Sí, ofrecemos hasta un 20% de descuento cuando pagas anualmente en lugar de mensualmente. Puedes ver el ahorro exacto seleccionando el período "Anual" arriba.',
  },
  {
    pregunta: '¿Hay límite de usuarios en cada plan?',
    respuesta:
      'Cada plan incluye un número base de usuarios. Puedes ver el límite específico en las características de cada plan. Si necesitas más usuarios, puedes contratar usuarios adicionales o cambiar a un plan superior.',
  },
  {
    pregunta: '¿Puedo probar antes de pagar?',
    respuesta:
      'Sí, ofrecemos períodos de prueba gratuitos en nuestros planes. Durante la prueba tendrás acceso completo a todas las funciones del plan seleccionado sin compromiso.',
  },
  {
    pregunta: '¿Cómo funciona la facturación?',
    respuesta:
      'La facturación es automática según el período que elijas (mensual, trimestral, semestral o anual). Recibirás tu factura por correo electrónico cada vez que se procese un cobro.',
  },
  {
    pregunta: '¿Qué soporte incluye cada plan?',
    respuesta:
      'Todos los planes incluyen soporte por correo electrónico. Los planes superiores incluyen soporte prioritario, chat en vivo y, en algunos casos, soporte telefónico dedicado.',
  },
];
...
function PlanesPublicPage() {
  ...
  const planes = useMemo(
    () => todosPlanes.filter((p) => p.tipo_cobro !== 'unico'),
    [todosPlanes]
  );

  // Separar planes recurrentes de únicos
  const planesRecurrentes = useMemo(
    () => planes.filter((plan) => plan.tipo_cobro !== 'unico'),
    [planes]
  );
  ...
  const calcularAhorro = (plan) => {
    ...
    const precioAnual =
      parseFloat(plan.precio_anual) || precioMensualAnualizado;
    ...
    <div className="min-h-screen bg-gradient-to-b from-gray-50 to-white dark:from-gray-900 dark:to-gray-800">
      ...
      <div className={cn('py-16 px-4 text-center', estadoBloqueo && 'pt-8')}>
        ...
        <p className="text-lg text-gray-600 dark:text-gray-400 max-w-2xl mx-auto">
          Comienza gratis y escala cuando lo necesites. Todos los planes
      ...
      <div className="max-w-7xl mx-auto px-4 pb-20">
        ...
        >
          {planes.map((plan) => {
            ...
            const precioMensual = parseFloat(plan.precio_mensual) || 0;
            const ahorro =
              !esUnico && periodoSeleccionado === CICLOS_FACTURACION.ANUAL
                ? calcularAhorro(plan)
                : 0;
                    ...
                    <>
                      <div className="flex items-baseline justify-center gap-1">
                        ...
                        <span className="text-gray-500 dark:text-gray-400">
                          /
                          {CICLO_LABELS[periodoSeleccionado]?.toLowerCase() ||
                            'mes'}
                        </span>
                      </div>

                      {/* Precio mensual equivalente para planes anuales */}
                      {periodoSeleccionado === CICLOS_FACTURACION.ANUAL &&
                        precioMensual > 0 && (
        ...
        <div className="max-w-3xl mx-auto">
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 overflow-hidden">
            {FAQ_ITEMS.map((item) => (
              <FAQItem
                ...
                onToggle={() =>
                  setFaqAbierto(
                    faqAbierto === item.pregunta ? null : item.pregunta
                  )
                }
              />
            ))}
          ...
          <div className="mt-8 text-center">
            <p className="text-gray-600 dark:text-gray-400 mb-4">
              ¿No encontraste lo que buscabas? Nuestro equipo está aquí para
              ayudarte.
            </p>
