...
function formatearFecha(fecha) {
  ...
  return fechaNotif.toLocaleDateString('es-MX', {
    ...
    month: 'short',
    year:
      fechaNotif.getFullYear() !== ahora.getFullYear() ? 'numeric' : undefined,
  });
}
...
const NotificacionListItem = memo(function NotificacionListItem({
  ...
  const nivelConfig =
    NOTIFICACION_NIVELES[notif.nivel] || NOTIFICACION_NIVELES.info;
  ...
  return (
    ...
    >
      <div className="flex gap-4">
        {/* Icono */}
        <div
          className={`flex-shrink-0 w-12 h-12 rounded-full flex items-center justify-center ${nivelConfig.bg}`}
        >
        ...
        <div className="flex-1 min-w-0 cursor-pointer" onClick={handleClick}>
          <div className="flex items-start justify-between gap-2 mb-1">
            <div>
              <p
                className={`font-medium ${!notif.leida ? 'text-gray-900 dark:text-gray-100' : 'text-gray-600 dark:text-gray-400'}`}
              >
                {notif.titulo}
              </p>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <label className="flex items-center gap-2 cursor-pointer">
            <input
              ...
              onChange={(e) =>
                handleCambiarFiltro('solo_no_leidas', e.target.checked)
              }
