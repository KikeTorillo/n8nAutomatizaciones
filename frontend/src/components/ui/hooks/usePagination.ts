...
export function normalizePagination(
  backendPagination?: BackendPagination | null
): PaginationInfo {
  ...
  const page =
    ...
    backendPagination.pagina ??
    backendPagination.pagina_actual ??
    1;
  const limit =
    backendPagination.limit ??
    backendPagination.limite ??
    backendPagination.elementos_por_pagina ??
    20;
  const total =
    backendPagination.total ?? backendPagination.total_elementos ?? 0;
  const totalPages =
    backendPagination.totalPages ??
    backendPagination.pages ??
    backendPagination.totalPaginas ??
    backendPagination.total_paginas ??
    Math.ceil(total / limit);
  const hasNext =
    backendPagination.hasNext ??
    backendPagination.tiene_siguiente ??
    page < totalPages;
  const hasPrev =
    backendPagination.hasPrev ?? backendPagination.tiene_anterior ?? page > 1;
