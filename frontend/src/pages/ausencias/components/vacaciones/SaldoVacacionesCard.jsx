  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 overflow-hidden">
      ...
      <div className="bg-gradient-to-r from-primary-600 to-primary-700 px-6 py-4">
        ...
        {nivel && (
          <p className="text-primary-100 text-sm mt-1">
            {nivel.nivel_nombre} ({formatDias(nivel.dias_correspondientes)}{' '}
            anuales)
          </p>
        )}
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 divide-y sm:divide-y-0 sm:divide-x divide-gray-200 dark:divide-gray-700">
        ...
        <div className="p-6 text-center">
          <div
            className={`text-3xl font-bold ${diasEnTramite > 0 ? 'text-yellow-600 dark:text-yellow-400' : 'text-gray-400'}`}
          >
      ...
      {progreso && progreso.aniosFaltantes && (
        <div className="px-6 py-4 bg-gray-50 dark:bg-gray-900/50 border-t border-gray-200 dark:border-gray-700">
          <div className="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-400 mb-2">
            ...
            <span>
              Próximo nivel en {progreso.aniosFaltantes.toFixed(1)} años
            </span>
            {progreso.diasSiguienteNivel && (
              <span className="text-primary-600 dark:text-primary-400">
                (+
                {progreso.diasSiguienteNivel -
                  (nivel?.dias_correspondientes || 0)}{' '}
                días)
              </span>
            )}
          </div>
