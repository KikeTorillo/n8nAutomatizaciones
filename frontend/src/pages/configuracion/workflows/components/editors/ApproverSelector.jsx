...
const TIPO_APROBADOR = {
  rol: {
    label: 'Por Rol',
    icon: Users,
    description: 'Cualquier usuario con el rol',
  },
  usuario: {
    label: 'Por Usuario',
    icon: User,
  ...
  permiso: {
    label: 'Por Permiso',
    icon: Shield,
    description: 'Usuarios con el permiso',
  },
  supervisor: {
    label: 'Por Superior',
    icon: UserCheck,
    description: 'Supervisor jerárquico del solicitante',
  },
};
      ...
      <div className="space-y-2">
        {ROLES_DISPONIBLES.map((rol) => (
          ...
          >
            ...
            `}
            >
              {value === rol.value && <Check className="w-3 h-3 text-white" />}
            </div>
...
function SupervisorSelector({ value = {}, onChange }) {
  ...
  return (
    <div className="space-y-4">
      <p className="text-sm text-gray-600 dark:text-gray-400 mb-3">
        El supervisor del usuario que inicie el workflow será asignado como
        aprobador automáticamente
      </p>
      ...
      <label className="flex items-center gap-3 p-3 rounded-lg border border-gray-200 dark:border-gray-700 cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
        ...
        <div>
          ...
          <p className="text-xs text-gray-500 dark:text-gray-400">
            Permite que cualquier supervisor en la cadena jerárquica apruebe, no
            solo el del nivel seleccionado
          </p>
        </div>
      </label>
      ...
      <div>
        ...
        <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
          Define qué sucede si el usuario no tiene un supervisor configurado en
          su perfil
        </p>
      </div>
      ...
      <div className="p-3 bg-primary-50 dark:bg-primary-900/20 rounded-lg border border-primary-200 dark:border-primary-800">
        <p className="text-sm text-primary-700 dark:text-primary-300">
          <strong>Nota:</strong> Para que esta opción funcione, los empleados
          deben tener un supervisor asignado en su perfil de profesional
          (Profesionales → Editar → Trabajo → Supervisor).
        </p>
      </div>
    </div>
  );
    ...
    <div className="space-y-4">
      ...
      <div className="min-h-[200px]">
        ...
        {tipoActivo === 'supervisor' && (
          <SupervisorSelector
            value={
              value.tipo === 'supervisor' ? value.supervisor_config || {} : {}
            }
            onChange={(config) =>
              onChange({
                tipo: 'supervisor',
      ...
      {(value.valor || value.tipo === 'supervisor') && (
        <div className="flex items-center gap-2 p-3 bg-green-50 dark:bg-green-900/20 rounded-lg">
          ...
          <span className="text-sm text-green-700 dark:text-green-300">
            {tipoActivo === 'rol' &&
              `Aprobador: Rol ${ROLES_DISPONIBLES.find((r) => r.value === value.valor)?.label}`}
            {tipoActivo === 'usuario' && `Aprobador: Usuario ID ${value.valor}`}
            {tipoActivo === 'permiso' && `Aprobador: Permiso ${value.valor}`}
            {tipoActivo === 'supervisor' &&
              `Aprobador: Supervisor ${value.supervisor_config?.cualquier_nivel ? '(cualquier nivel)' : `nivel ${value.supervisor_config?.nivel || 1}`}`}
          </span>
          <button
            type="button"
            onClick={() => {
