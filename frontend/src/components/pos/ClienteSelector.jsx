    ...
    return (
      <div className="flex items-center gap-2 px-3 py-2 bg-cyan-50 border border-cyan-200 rounded-lg">
        ...
        <div className="flex-1 min-w-0">
          <p className="text-sm font-medium text-cyan-900 truncate">{value.nombre}</p>
          {value.telefono && (
            <p className="text-xs text-cyan-600 truncate">{value.telefono}</p>
          )}
        </div>
        {!disabled && (
          <button
            type="button"
            onClick={handleClear}
            className="p-1 text-cyan-500 hover:text-cyan-700 hover:bg-cyan-100 rounded transition-colors"
          >
            <X className="w-4 h-4" />
          </button>
        )}
    ...
    <div className="relative" ref={dropdownRef}>
      ...
      {isOpen && !disabled && (
        <div className="absolute z-50 w-full mt-1 bg-white border border-gray-200 rounded-lg shadow-lg overflow-hidden">
          ...
          {showCreateForm ? (
            <form onSubmit={handleCreateCliente} className="p-3 border-t border-gray-100 bg-gray-50">
            ...
            <div className="p-2 border-t border-gray-100">
              <button
                ...
                className="w-full px-3 py-2 flex items-center justify-center gap-2 text-sm text-cyan-600 hover:bg-cyan-50 rounded-lg transition-colors"
              >
