...
export const ViewTabs = memo(function ViewTabs({
  ...
  return (
    <div className={cn(TAB_CONTAINER_STYLES, className)}>
      <nav className={TAB_NAV_STYLES} role="tablist" aria-label={ariaLabel}>
        {tabs.map((tab, index) => (
          <TabButton
            key={tab.id}
            ref={(el) => {
...
const TabButton = memo(
  forwardRef<HTMLButtonElement, TabButtonProps>(function TabButton(
    ...
    return (
      ...
      >
        {Icon && (
          <Icon className={getTabIconStyles(isActive)} aria-hidden="true" />
        )}
