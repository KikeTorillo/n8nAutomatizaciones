...
const AdvancedFilterPanel = memo(
  forwardRef<HTMLDivElement, AdvancedFilterPanelProps>(
            ...
            return (
              <FilterCheckbox
                ...
                onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                  handleFilterChange(config.id, e.target.checked)
                }
          ...
          case 'checkbox-group':
            return (
              <div key={config.id} className="space-y-1">
                {(config.options as CheckboxGroupOption[] | undefined)?.map(
                  (opt) => (
                    <FilterCheckbox
                ...
                <div className="space-y-4">
                  <FilterSection title="Filtros" icon={Filter}>
                    {filterConfig
                      .filter(
                        (c) =>
                          c.type === 'checkbox' || c.type === 'checkbox-group'
                      )
                      .map(renderFilter)}
                  </FilterSection>
                </div>
                  ...
                  <FilterSection title="Favoritos" icon={Star}>
                    <SavedSearchList
                      ...
                      onDelete={
                        onEliminarBusqueda
                          ? (id: string | number) => {
                              const busqueda = busquedasGuardadas.find(
                                (b) => b.id === id
                              );
