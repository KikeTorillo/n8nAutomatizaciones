...
function UsuarioUbicacionesTab({ usuarioId, canEdit = true }) {
  ...
  // Fetch ubicaciones
  const { data: ubicaciones = [], isLoading } =
    useUbicacionesUsuario(usuarioId);
  ...
  const handleDesasignar = async (ubicacionId, ubicacionNombre) => {
    if (!window.confirm(`¿Desasignar la ubicación "${ubicacionNombre}"?`))
      return;
      ...
      {ubicaciones.length === 0 ? (
        <div className="text-center py-12">
          ...
          <p className="text-sm text-gray-500 dark:text-gray-500 mb-4">
            Las ubicaciones se asignan automáticamente cuando el usuario es
            asignado a una sucursal, o puedes asignarlas manualmente.
          </p>
          {canEdit && (
            <Button
              size="sm"
                  ...
                  >
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <div className="flex items-center gap-1">
                        <Badge
                          variant={
                            ubicacion.puede_recibir ? 'success' : 'secondary'
                          }
                          size="sm"
                          title={
                            ubicacion.puede_recibir
                              ? 'Puede recibir'
                              : 'No puede recibir'
                          }
                        >
                        ...
                        <Badge
                          variant={
                            ubicacion.puede_despachar ? 'success' : 'secondary'
                          }
                          size="sm"
                          title={
                            ubicacion.puede_despachar
                              ? 'Puede despachar'
                              : 'No puede despachar'
                          }
                        >
                            ...
                            <>
                              ...
                              <div className="absolute right-0 mt-1 w-48 bg-white dark:bg-gray-800 rounded-lg shadow-lg border border-gray-200 dark:border-gray-700 z-20">
                                ...
                                >
                                  ...
                                  {ubicacion.puede_despachar
                                    ? 'Quitar permiso despachar'
                                    : 'Dar permiso despachar'}
                                </button>
