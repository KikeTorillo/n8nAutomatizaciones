...
class MotivoSalidaModel {
    ...
    static async listar(organizacionId, filtros = {}) {
        ...
        if (solo_sistema) {
            ...
            whereClauses.push('organizacion_id = $1');
            whereParams.push(organizacionId);
        } else {
            whereClauses.push('(organizacion_id IS NULL OR organizacion_id = $1)');
            if (organizacionId) {
                whereParams.push(organizacionId);
            }
        }
        ...
        const { rows } = await RLSContextManager.query(query, whereParams);
        return rows;
    }
    ...
    static async estadisticas(organizacionId) {
        ...
        const { rows } = await RLSContextManager.query(query, [organizacionId]);
        return rows;
    }
    ...
    static async obtenerPorId(organizacionId, motivoId) {
        ...
        const { rows } = await RLSContextManager.query(query, [motivoId, organizacionId]);
        return rows[0] || null;
    }
    ...
    static async obtenerPorCodigo(organizacionId, codigo) {
        ...
        const { rows } = await RLSContextManager.query(query, [codigo, organizacionId]);
        return rows[0] || null;
    }
    ...
    static async crear(organizacionId, motivoData) {
        ...
        const { rows } = await RLSContextManager.query(query, [
            organizacionId,
            codigo,
            nombre,
            descripcion,
    ...
    static async actualizar(organizacionId, motivoId, motivoData) {
        ...
        const { rows } = await RLSContextManager.query(query, values);
        return rows[0] || null;
    }
    ...
    static async estaEnUso(motivoId) {
        ...
        const { rows } = await RLSContextManager.query(query, [motivoId]);
