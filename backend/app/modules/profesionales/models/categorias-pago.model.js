 ...
 * @date Enero 2026
 *
 * GAP-004 vs Odoo 19: Clasificación de empleados para compensación
 */
...
class CategoriaPagoModel {
    ...
    static async obtenerPorId(organizacionId, categoriaId) {
        ...
        const { rows } = await RLSContextManager.query(query, [categoriaId, organizacionId]);
        return rows[0] || null;
    }
    ...
    static async buscarPorCodigo(organizacionId, codigo) {
        ...
        const { rows } = await RLSContextManager.query(query, [organizacionId, codigo]);
        return rows[0] || null;
    }
    ...
    static async estaEnUso(categoriaId) {
        ...
        const { rows } = await RLSContextManager.query(query, [categoriaId]);
