 ...
 */
const SolicitudCard = memo(function SolicitudCard({
  solicitud,
  onVerDetalle,
  onCancelar,
}) {
...
const SolicitudesVacacionesList = memo(function SolicitudesVacacionesList({
  ...
  const handleCancelar = async (solicitud) => {
    if (
      window.confirm('¿Estás seguro de cancelar esta solicitud de vacaciones?')
    ) {
    ...
    return (
      <div className="bg-gray-50 dark:bg-gray-900/50 rounded-lg p-8 text-center">
        ...
        <p className="text-gray-500 dark:text-gray-400">
          ...
          {filtroEstado &&
            ` con estado "${getEstadoSolicitud(filtroEstado).label}"`}
        </p>
      </div>
    );
  }
