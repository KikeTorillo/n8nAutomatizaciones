...
function ServiciosCanvasBlock({ bloque, tema, isEditing, onContentChange }) {
  ...
  const servicios = useMemo(() => {
    if (origen === 'erp') {
      return serviciosERPMapped.length > 0
        ? serviciosERPMapped
        : [
            {
              icono: 'Database',
              nombre: 'Cargando servicios...',
              descripcion: 'Desde el módulo de Servicios',
              precio: 0,
            },
          ];
    }
          ...
          {subtitulo_seccion &&
            (isEditing ? (
              <InlineText
                ...
                onChange={(value) =>
                  onContentChange({ subtitulo_seccion: value })
                }
        ...
        <div className={cn('grid gap-8', gridCols[colKey] || gridCols['3'])}>
          {servicios.map((servicio, index) => {
            ...
            const IconComponent = servicio.icono
              ? getDynamicIcon(servicio.icono, Star)
              : null;
